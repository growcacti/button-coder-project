...
class Coder(object):
    ...
    def __init__(self,parent):
        ...
        self.btn25 = tk.Button(self.frm1, text="Methods open/save", bg="violet",bd =8, command=self.open_save)
        self.btn25.grid(row=5, column=2)
        self.btn26 = tk.Button(self.frm1, text="Generic Toolbar", bg="orange",bd=8, command=self.toolbar1)
        self.btn26.grid(row=6, column=2)
        self.btn27 = tk.Button(self.frm1, text=" ", bg="orange",bd=6, command=None)
        ...
        self.btn_oo = tk.Button(self.frm1, text="Save As...", command=lambda:self.ssavestr)
        self.btn_oo.grid(row=2, column=4)
...
self.label1.grid(row=5, column=2)"""
    ...
    def save(self):
      ...
      """)
        ...
        self.txt.insert(tk.END, open_savestr)
...
class ToolBar(object):
    ...
    def __init__(self, parent):
        ...
        self.canvas = tk.Canvas(self.parent, height=25, width=800, background="cornsilk")
        #self.canvas.configure( )
        self.canvas.grid(row=3, column=0)
        #self.canvas.create_line(x1, y1, x2, y2, fill="black" width=1)
        self.cb1 = ttk.Combobox(self.canvas, values=["Value1", "value2", "value3"])
        self.cb1.grid(column=1, row=1)

        self.sp1 = tk.Spinbox(self.canvas, from_=1.0, to=1000.0, increment=0.1)
        self.sp1.grid(row=1, column=2)

        self.b1 = tk.Button(self.canvas,relief=tk.FLAT, compound=tk.LEFT,text="new",bd=6,command=None)
